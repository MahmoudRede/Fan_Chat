...
import 'package:fanchat/data/modles/fanArea_model.dart';
...
class AppCubit extends Cubit<AppState> {
  ...
  FanModel? fanModel;
  Future getFanArea() async {
    FirebaseFirestore.instance
        .collection('images')
        .doc(AppStrings.uId)
        .get()
        .then((value) {
      fanModel = FanModel.formJson(value.data()!);
      printMessage('${fanModel!.name}');
      //emit(GetFanAreaSuccessfulState());
    }).catchError((error) {
      printMessage('Error in Fan Area is ${error.toString()}');
      //emit(GetFanAreaErrorState());
    });
  }
